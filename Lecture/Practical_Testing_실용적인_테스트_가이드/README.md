...
핵심 과정은
레드, 그린, 리팩토링 이라는 과정을 거친다
레드 : 실패하는 테스트를 작성함.(구현 부가 없기 때문에)
...
이 작업을 통해 우리는 다음을 가치 있게 여기게 되었가.
...
엔티티 설계
Order와 Product는 다대다 관계다.
그래서 Order, OrderProduct, Product와 같은 방식으로 일대다 다대일 관계로 풀어내서 진행.
...
Persistence Layer를 테스트할 땐
=> @DataJpaTest 어노테이션 또는 @SpringBootTest로 테스트 할 수 있지만 강사님은 @SpringBootTest를 선호한다고 하심.
...
보통 개발을 할 때 컨트롤러에서 넘어온 인자의 값을 검증하기 위해 @Valid를 많이 쓴다.
@Valid에서 지원하는 메소드들로 Dto에 어노테이션으로 필드값 검증을 달아놓는데,
예를들어 닉네임은 20자 이내로 작성하여야 한다는 비즈니스가 있다면 이 검증을 과연 Dto에서 해주어야 할까?
이러한 비즈니스는 엔티티 내부 생성자에서 해주거나, 서비스 레이어에서 검증을 해주는 것이 맞다고 보는 부분!
...
메일 전송이라는 기능을 넣고 테스트를 하려고 한다. 이럴 때 메일 전송 기능은 외부 네트웍을 사용하기 때문에,
기능이 정상적으로 동작하는지 테스트하려고 한다면 메일을 실제로 보내보고 잘 전송되는지 매번 테스트하여야 할까?
...
차이점 : @WebMvcTest는 @Controller, @ControllerAdvice 등 사용이 가능하지만, @Service, @Repository에는 사용 불가
                                @MockBean을 사용하여 컨트롤러의 협력체들을 생성한다.
                                주로 간단한 테스트에서 사용한다.
            @AutoConfigureMockMvc는 컨트롤러 뿐 아니라 @Service, @Repository
